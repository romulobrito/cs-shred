...
def plot_csshred_results(
    ...
    test_ground_truth_test = test_ground_truth_test.reshape(-1, dim_x, dim_y)
    ...
    img1 = ax1.imshow(test_recons[-1, :, :], cmap="coolwarm", origin="lower")
    fig1 = plt.gcf()
    cbar1 = fig1.colorbar(img1, ax=ax1, label=r"$q_{max}$")
    plt.title("Reconstructed - Last Snapshot")
    
    ax2 = plt.subplot(1, 2, 2)
    img2 = ax2.imshow(test_ground_truth_test[-1, :, :], cmap="coolwarm", origin="lower")
    fig2 = plt.gcf()
    cbar2 = fig2.colorbar(img2, ax=ax2, label=r"$q_{max}$")
    ...
    img3 = ax3.imshow(test_recons[-1, :, :], cmap="coolwarm", origin="lower")
    fig3 = plt.gcf()
    cbar3 = fig3.colorbar(img3, ax=ax3, label=r"$q_{max}$")
    plt.title("Reconstructed - Last Snapshot")
    
    ax4 = plt.subplot(1, 2, 2)
    img4 = ax4.imshow(subsampled[:, :, -1], cmap="coolwarm", origin="lower")
    fig4 = plt.gcf()
    cbar4 = fig4.colorbar(img4, ax=ax4, label=r"$q_{max}$")
    ...
    mse_value = np.linalg.norm(test_recons - test_ground_truth_test) / np.linalg.norm(test_ground_truth_test)
    
    mse = mean_squared_error(test_ground_truth_test[-1, :, :], test_recons[-1, :, :])
    max_pixel = np.max(test_ground_truth_test[-1, :, :])
    psnr_value = 20 * log10(max_pixel / np.sqrt(mse))
    
    # Calculate normalized error
    error_norm = np.linalg.norm(test_recons[-1, :, :] - test_ground_truth_test[-1, :, :]) / np.linalg.norm(test_ground_truth_test[-1, :, :])
    
    print(f"SSIM for the last snapshot: {ssim_value}")
    print(f"MSE for the last snapshot: {mse_value}")
    print(f"PSNR for the last snapshot: {psnr_value} dB")
    print(f"Normalized Error for the last snapshot: {error_norm}")
    ...
    results = {
        "SSIM_last_snapshot": float(ssim_value),
        "MSE_last_snapshot": float(mse_value),
        "PSNR_last_snapshot": float(psnr_value),
        "Normalized_Error_last_snapshot": float(error_norm),  
...
def plot_csshred_results_cartography(
    train_error, validation_errors, test_recons, test_ground_truth_test, matrix, subsampled,
    save_path=r'./results/csshred/qmax/'
):
    """
    Plot cartography results for CS-SHRED model with corrected data handling.
    ...
    plt.title("Training Error Over Epochs")
    plt.legend()
    training_error_plot_path = os.path.join(save_path, "training_error_cartography.pdf")
    plt.savefig(training_error_plot_path)
    plt.show()
    ...
    plt.title("Validation Error Over Epochs")
    plt.legend()
    validation_error_plot_path = os.path.join(save_path, "validation_error_cartography.pdf")
    plt.savefig(validation_error_plot_path)
    plt.show()
    ...
    last_snapshot_plot_path = os.path.join(save_path, "last_snapshot_comparison_cartography.pdf")
    plt.savefig(last_snapshot_plot_path)
    plt.show()
    ...
    subsampled_snapshot_plot_path = os.path.join(save_path, "subsampled_snapshot_comparison_cartography.pdf")
    plt.savefig(subsampled_snapshot_plot_path)
    plt.show()
    ...
    json_file_path = os.path.join(save_path, "results_cartography.json")
    with open(json_file_path, "w") as json_file:
        json.dump(results, json_file, indent=4)
...
def plot_shred_results(
    ...
    save_path=r'./results/shred/qmax/'
):
    """
...
def plot_shred_results_cartography(
    validation_errors, test_recons, test_ground_truth_test, matrix, subsampled,
    save_path=r'./results/shred/qmax/'
):
    """
    Plot cartography results for SHRED model with corrected data handling.
    ...
    plt.title("Validation Error Over Epochs")
    plt.legend()
    validation_error_plot_path = os.path.join(save_path, "validation_error_cartography.png")
    plt.savefig(validation_error_plot_path)
    plt.show()
    ...
    last_snapshot_plot_path = os.path.join(save_path, "last_snapshot_comparison_cartography.png")
    plt.savefig(last_snapshot_plot_path)
    plt.show()
    ...
    subsampled_snapshot_plot_path = os.path.join(save_path, "subsampled_snapshot_comparison_cartography.png")
    plt.savefig(subsampled_snapshot_plot_path)
    plt.show()
...
def main():
    ...
    """
    print("=== QMAX PLOTTING SCRIPT (CORRECTED) ===")
    
    # CS-SHRED Results
    print("\n--- CS-SHRED Results ---")
    csshred_save_path = r"./results/csshred/qmax"
    csshred_load_path = r"./results/csshred/qmax"
    ...
    try:
        ...
        # Plot CS-SHRED cartography results
        plot_csshred_results_cartography(
            train_error, validation_errors, test_recons, test_ground_truth_test, matrix, snapshot,
            save_path=csshred_save_path
        )
    ...
    # SHRED Results
    print("\n--- SHRED Results ---")
    shred_save_path = r"./results/shred/qmax"
    shred_load_path = r"./results/shred/qmax"
    ...
    try:
        ...
        # Plot SHRED cartography results
        plot_shred_results_cartography(
            validation_errors, test_recons, test_ground_truth_test, matrix, snapshot,
            save_path=shred_save_path
        )
