...
def objective(trial):
    ...
    matrix = load_data(npy_file_path, 0)

    model_type = 'CS-SHRED'

    # Parâmetros
    percent_cols_subsample = 0.9
    percent_snapshots_subsample = 0.3
    seed = 915

    hidden_size = trial.suggest_categorical("hidden_size", [64, 128, 256, 512])
    hidden_layers = trial.suggest_categorical("hidden_layers", [1,2,3,4,5])
    ...
    num_epochs = trial.suggest_int("num_epochs", 100, 800)  
    ...
    step_epoch = trial.suggest_int("step_epoch", 10, 100)
    ...
    print("step_epoch=", step_epoch)  # Imprime step_epoch
    ...
    snapshot = subsample(snapshot, percent_cols_subsample, percent_snapshots_subsample)
    ...
    if model_type == 'CS-SHRED':
        ...
        train_error, validation_errors = models.fit_csshred_model(
            ...
            lambL1=lambL1,
            lambdaSNR=lambdaSNR,
            step_epoch=step_epoch,  # Incluindo step_epoch
            verbose=False,
            patience=patience,
        )
    else:
...
best_params = {
    ...
    "step_epoch": int(trial.params.get("step_epoch")),
}
